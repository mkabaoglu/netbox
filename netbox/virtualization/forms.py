...
from utilities.forms import (
    ...
    DynamicModelMultipleChoiceField, ExpandableNameField, form_from_model, JSONField, SlugField, SmallTextarea, NumericArrayField,
...
class ServicesForm(BootstrapMixin, CustomFieldModelForm):
    cluster = DynamicModelChoiceField(
        queryset=Cluster.objects.all(),
        query_params={
            'group_id': '$cluster_group'
    ...
    class Meta:
        model = Services
        fields = [
            'cluster','name', 'protocol', 'ports', 'Domain', 'description', 'tags',
        ]
...
class ServicesFilterForm(BootstrapMixin, CustomFieldFilterForm):
    model = Services
    q = forms.CharField(
        required=False,
        label='Search'
    ...
    protocol = forms.ChoiceField(
        choices=add_blank_choice(ServicesProtocolChoices),
        required=False,
        widget=StaticSelect2Multiple()
    )
...
class ServicesCSVForm(CustomFieldModelCSVForm):
    device = CSVModelChoiceField(
        queryset=Cluster.objects.all(),
        required=False,
        to_field_name='name',
        help_text='Required if not assigned to a Cluster'
    )
    protocol = CSVChoiceField(
        choices=ServicesProtocolChoices,
        help_text='IP protocol'
    )

    class Meta:
        model = Services
        fields = Services.csv_headers


class ServicesBulkEditForm(BootstrapMixin, AddRemoveTagsForm, CustomFieldBulkEditForm):
    model = Services
    pk = forms.ModelMultipleChoiceField(
